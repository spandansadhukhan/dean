...
<div class="wrapper">
    ...
    <div class="row gap_row">
        <div class="col-sm-12">
      <section class="panel">
        <header class="panel-heading"> Templates (Total Records: <?php echo $total_user;?>) </header>
        ...
        <div class="table-options clearfix" style=" background-color:#fff">
          <div class="clearfix pull-left" style="display:<?php if(isset($this->request->data['Filter']['advace_search_type']) and $this->request->data['Filter']['advace_search_type']==1){?>none; <?php }else{ ?>block;<?php } ?>" id="filterInput"> &nbsp;
            ...
            <div class="btn-group btn-group-sm pull-left">
              <div class="input-group"> <?php echo $this->Form->input("name",array('class'=>'form-control col-md-6','placeholder'=>'Search by display name or username..','label'=>false))?> <span class="input-group-btn">
                <button class="btn btn-primary" type="submit" name="search_single">Search</button>
                </span> </div>
            </div>
          </div>
          <div class="btn-group btn-group-sm pull-right"> <a id="style-hover" class="btn btn-primary gap_up gap_up_down" title="" data-toggle="tooltip" href="<?php echo $this->webroot;?>admin/users/addwebsitetemplate" data-original-title="Add New Template">Add New</a> </div>
        </div>
        <?php echo $this->Form->end();?>
        <div style=" clear:both;"></div>
        <?php echo $this->Form->create("Filter",array('class' => 'filter'));?> <?php echo $this->Form->input("advace_search_type",array('type'=>'hidden','value'=>1))?>
            ...
            <div class="block full">
              ...
              <div class="form-inline">
                <div class="form-group">
                  <label for="example-if-email" class=""> Total Search Key</label>
                  <br>
                  <?php echo $this->Form->input("name",array('class'=>'form-control','placeholder'=>'Search by display name or username..','label'=>false,'div'=>false))?> </div>
                <div class="form-group">
                  <label for="example-if-email"> Show Me</label>
                  <br>
                  <?php echo $this->Form->input("is_active",array('options'=>$user_status,'class'=>'select-chosen', 
                  'data-placeholder'=>'Choose a Type..','style'=>'width: 250px', 
                  'selected'=>!empty($this->request->data['Filter']['is_active'])?$this->request->data['Filter']['is_active']:'', 
                ...
                <div class="form-group">
                  ...
                  <br>
                  <?php echo $this->Form->input("direction",array('options'=>$directions,'empty'=>'All','class'=>'select-chosen', 
                  'data-placeholder'=>'Choose a Type..','style'=>'width: 250px', 
                  'selected'=>!empty($this->request->data['Filter']['direction'])?$this->request->data['Filter']['direction']:'', 
                  'label'=>false,'div'=>false));
                  ?>
                </div>
                <div class="form-group">
                ...
                <div class="adv-table col-sm-12">
	                ...
	                <?php foreach ($name as $user): ?>
                            <tr class="gradeX">
                                <td><?php echo $user['WebsiteTemplate']['id']; ?>&nbsp;</td>
                                <td><?php echo $user['WebsiteTemplate']['name']; ?>&nbsp;</td>
                                <td><img src="<?php echo $this->webroot?>img/<?php echo $user['WebsiteTemplate']['template']; ?>" height="100px" width="100px"></td>
                                <td><?php if($user['WebsiteTemplate']['is_paid'] == 0){echo 'Free';}else{echo 'Paid';} ?>&nbsp;</td>
                                 <td><?php echo $user['WebsiteTemplate']['price'];?>&nbsp;</td>
                                 <td><?php if($user['WebsiteTemplate']['is_active'] == 0){echo 'Inactive';}else{echo 'Active';} ?>&nbsp;</td>
                                <td>

                                    <div class="btn-group btn-group-sm">  

                                        <a href="javascript:void(0);" onclick="getViewUser(<?php echo $user['WebsiteTemplate']['id']; ?>)" class="btn btn-default enable-tooltip view-record" data-original-title="View Record"><i class="fa fa-search"></i></a>

                                        <a href="<?php echo $this->webroot;?>admin/users/editwebsitetemplate/<?php echo $user['WebsiteTemplate']['id']; ?>" class="btn btn-info enable-tooltip" data-original-title="Manage Account"><i class="fa fa-sign-in"></i></a>

                                        

          <?php if($user['WebsiteTemplate']['is_active']==1){ ?>
                                        <a href="javascript:void(0);"  onclick="active(<?php echo $user['WebsiteTemplate']['id'];?>)" class="btn btn-success enable-tooltip" data-original-title="Inactive Record"><i class="fa fa-circle-o"></i></a>

          <?php } else if($user['WebsiteTemplate']['is_active']==0){ ?>
                                        <a href="javascript:void(0);" onclick="Inactive(<?php echo $user['WebsiteTemplate']['id'];?>)" class="btn btn-warning enable-tooltip" data-original-title="Inactive Record"><i class="fa fa-circle-o"></i></a>
          <?php } ?>
...
<?php } ?>
    ...
    function active(aa) {
        ...
        if (a)
        {
            location.href = "<?php echo $this->webroot?>admin/users/activetemplate/" + aa;
        }
    }
 ...
 function getViewUser(userid) {
        //alert(userid);
        $.post('<?php echo($this->webroot)?>admin/users/templateview/' + userid, function (data) {
