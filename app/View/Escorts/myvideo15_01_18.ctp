...
<script>
    $(function () {
        ...
        $max_upload=parseInt(<?php echo $max_img_upload;?>);
        $user_upload=parseInt(<?php echo $user_uploaded_video;?>);
        $("#form1").submit(function(event){
        if($user_upload>=$max_upload)
        {
            
            $("#videoupload").modal("show");
                                                                ...
                                                                <form action="" method="post" accept-charset="utf-8" class="" id="form1" enctype="multipart/form-data">                  
                                                                    <section class="upc-tom1">
                                                                        <div id="uploader" style="max-width:900px;">
                                                                            <form action="" method="post" accept-charset="utf-8" class="ajaxform" id="foema" enctype="multipart/form-data">						<input type="hidden" name="uid" value="<?php echo $user['Escort']['id']?>" /> 
...
<script>
    function deleteVideo(id) {
        $.ajax({
            type: "POST",
            url: "<?php echo $this->Html->url('/'); ?>escorts/deletevideo/",
            //dataType: "json",
            data: {id: id}
        }).done(function (msg) {          
            $("#image_"+id).hide();
            window.location.href = "<?php echo Router::url(array('controller' => 'escorts', 'action' => 'myvideo')); ?>";
        });
    }
