...
?>
  <div class="wrapper">
    ...
    <div class="row gap_row">
      <div class="col-sm-12">
        ...
        <header class="panel-heading">
            Agencies (Total Records: <?php echo $total_user;?>)
        </header>
           ...
           <?php echo $this->Form->input("advace_search_type",array('type'=>'hidden','value'=>0))?>
            <div class="table-options clearfix" style=" background-color:#fff">
                ...
                <div class="btn-group btn-group-sm pull-right">
                    <a id="style-hover" class="btn btn-primary gap_up gap_up_down" title="" data-toggle="tooltip" href="<?php echo $this->webroot;?>admin/users/addescort" data-original-title="Add New Agency">Add New</a>
                </div>
        </div>
    ...
    <!-- <th><?php echo 'Featur'; ?></th> -->
        
         <th><?php echo 'Email Approved'; ?></th>
    ...
    <td>
      ...
      {
      ?>
      <img src="<?php echo $this->webroot;?>user_images/<?php echo $user['User']['profile_image']; ?>" style="height:100px;width:100px;">&nbsp;
      <?php
      }
      else
      {  
    ...
    <!-- <td><?php echo 0; ?>&nbsp;<?php echo $user['User']['is_active']; ?></td>   -->
    <td><?php if($user['User']['is_active']==1) {echo 'Active';} else { echo 'Inactive';} ?>&nbsp;</td>
     <td><?php if($user['User']['is_email_verified']==1) {echo 'Approved';} else { echo 'Not Approved';} ?>&nbsp;</td>
...
<!--                <td class="text-center">
                ...
                <a href="<?php echo $this->webroot;?>admin/agencies/delete/<?php echo $user['User']['id']; ?>" onclick="del(<?php echo $user['User']['id'];?>)" class="btn btn-default enable-tooltip view-record btn btn-primary" data-original-title="View Record"><i class="fa fa-times" aria-hidden="true"></i></a>
                       ...
                       <td class="text-center">
                                    <div class="btn-group btn-group-sm">  
                                        ...
                                        <a href="<?php echo $this->webroot;?>admin/escorts/delete/<?php echo $user['User']['id']; ?>" onclick="del(<?php echo $user['User']['id'];?>)" class="btn btn-danger enable-tooltip" data-original-title="Delete Record"><i class="fa fa-times"></i></a>
            
          <?php if($user['User']['is_active']==1){ ?>
                                        <a href="javascript:void(0);"  onclick="active(<?php echo $user['User']['id'];?>)" class="btn btn-success enable-tooltip" data-original-title="Inactive Record"><i class="fa fa-check-circle"></i></a>
          ...
          <?php } ?>
                                        ...
                                        <a href="<?php echo $this->webroot;?>admin/escorts/edit/<?php echo $user['User']['id']; ?>" class="btn btn-info enable-tooltip" data-original-title="Manage Account"><i class="fa fa-sign-in"></i></a>


          <?php if($user['User']['is_email_verified']==1){ ?>
                                        <a href="javascript:void(0);"  onclick="active_email(<?php echo $user['User']['id'];?>)" class="btn btn-success enable-tooltip" data-original-title="Inactive Record"><i class="fa fa-upload"></i></a>

          <?php } else if($user['User']['is_email_verified']==0){ ?>
                                        <a href="javascript:void(0);" onclick="Inactive_email(<?php echo $user['User']['id'];?>)" class="btn btn-warning enable-tooltip" data-original-title="Inactive Record"><i class="fa fa-upload"></i></a>
          <?php } ?>
                                        <!--fa fa-life-bouy-->
                                    </div>
                                </td>
...
function active(aa) {
        ...
        if (a)
        {
            location.href = "<?php echo $this->webroot?>admin/users/activeescort/" + aa;
        }
    }
    ...
    function active_email(aa) {

        var a = confirm("  Are you sure, you want to cancle the approval of this record?")
        if (a)
        {
            location.href = "<?php echo $this->webroot?>admin/users/verifyescortemail/" + aa;
        }
    }

    function Inactive_email(aa) {
