					...
					<div class="uploadButton mb-2">
						<section class="upc clear">
                                                                <form action="" method="post" accept-charset="utf-8" class="" id="form1" 
                                                                    ...
                                                                    <input type="hidden" name="uid" value="<?php echo $user['Stripper']['id']?>" />
    ...
    function deletephoto(id) {
        $.ajax({
            type: "POST",
            url: "<?php echo $this->Html->url('/'); ?>strippers/deletephoto/",
            //dataType: "json",
            data: {id: id}
        }).done(function (msg) {          
            $("#image_"+id).hide();
            window.location.href = "<?php echo Router::url(array('controller' => 'strippers', 'action' => 'mypphoto')); ?>";
        });
    }
</script>
