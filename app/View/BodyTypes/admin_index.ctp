				...
				<div class="col-sm-9 col-lg-2">
                 ...
                 <div class="col-sm-9 col-lg-2">
                            ...
                            <a href="<?php echo $this->webroot; ?>admin/body_types" class=" widget-hover-effect1" style="text-decoration:none">
                                <button class="btn btn-sm btn-success btn-block" type="button" style="font-size:20px;margin-bottom:10px;">Body Types</button>
                            </a>
...
-->
    ...
    <div class="row gap_row">
      <div class="col-sm-12">
        ...
        <header class="panel-heading">
            Body Type  (Total Records: <?php echo $total_bodytype; ?>)
        </header>
            ...
            <div class="table-options clearfix" style=" background-color:#fff">
				...
				<div class="btn-group btn-group-sm pull-left">
						<div class="input-group">
                                                     <?php echo $this->Form->input("body_type",array("type"=>"text",'class'=>'form-control col-md-6','placeholder'=>'Search by  name ...','label'=>false))?>
                ...
                <div class="btn-group btn-group-sm pull-right">
					<a id="style-hover" class="btn btn-primary gap_up gap_up_down" title="" data-toggle="tooltip" href="<?php echo $this->webroot;?>admin/body_types/add" data-original-title="Add BodyType">Add New</a>
				</div>
       ...
       <div class="block-title">
        <div class="block-options pull-right form-actions">
            <button class="btn btn-alt btn-sm btn-danger enable-tooltip" id="advanceSearch_close" type="button" data-original-title="Advance Search close"  data-toggle="tooltip" data-placement="top" title="Advance Search Close" onclick="advance_close();"><i class="fa fa-times"></i></button>
             <button class="btn btn-alt btn-sm btn-warning enable-tooltip" onclick="location.href='<?php echo $this->webroot;?>admin/body_types'" type="button" data-original-title="Reset Search" data-toggle="tooltip" data-placement="top" title="Reset Search"><i class="fa fa-repeat"></i></button>
        </div>
       <h2><strong>Advance </strong>Search</h2>
                               ...
                               <div class="form-inline">
                                    <div class="form-group">
                                        ...
                                        <?php echo $this->Form->input("body_type",array('type'=>"text",'class'=>'form-control','placeholder'=>'Search by  name...','label'=>false,'div'=>false))?>
	...
	<tr>
        ...
        <tbody>
	<?php foreach ($bodyTypes as $content):
	    
	    ?>
        <tr class="gradeX">
        <td><input type="checkbox" class="changestatus" value="<?php echo $content["BodyType"]["id"]; ?>"></td>
        <td><?php echo $content["BodyType"]["id"]; ?></td>
        <td><?php echo $content["BodyType"]["body_type"]; ?></td>
        <td><?php echo $content["BodyType"]["is_active"]?"Active":"Inactive"; ?></td>
        <td><?php echo date("Y-m-d",strtotime($content["BodyType"]["posttime"])); ?></td>
        <td class="text-center">
        <div class="btn-group btn-group-sm">
            <a href="javascript:void(0);" onclick="viewad(<?php echo $content['BodyType']['id']; ?>)" class="btn btn-default enable-tooltip view-record" data-original-title="View Record"><i class="fa fa-search"></i></a>
            <a href="<?php echo $this->webroot;?>admin/body_types/edit/<?php echo $content['BodyType']['id']; ?>" class="btn btn-default enable-tooltip view-record btn btn-primary" data-original-title="edit Record"><i class="fa fa-pencil" aria-hidden="true"></i></a>                
           <a href="javascript:void(0)" onclick="del(<?php echo $content['BodyType']['id'];?>)" class="btn btn-default enable-tooltip view-record btn btn-primary" data-original-title="View Record" onclick="bulk_delete()"><i class="fa fa-times" aria-hidden="true"></i></a>


 <?php 
        if($content["BodyType"]["is_active"]==0)
        {
        ?>
        <a href="<?php echo $this->webroot; ?>admin/body_types/changestatus/<?php echo $content['BodyType']['id']; ?>/1" class="btn btn-default enable-tooltip view-record" data-original-title="Active" 
        style="background-color:#f7be64;border-color:#f7be64; display: inline-block;"><i class="fa fa-circle-o"></i></a>
        <?php }else{?>
       <a href="<?php echo $this->webroot; ?>admin/body_types/changestatus/<?php echo $content['BodyType']['id']; ?>/0" class="btn btn-default enable-tooltip view-record" data-original-title="Inactive"
...
function viewad(userid){
  //alert(userid);
	$.get('<?php echo($this->webroot)?>admin/body_types/view/'+userid,function(data){
...
function del(aa){
  var a=confirm("Are you sure, you want to delete this?")
      if (a)
      {
        location.href="<?php echo $this->webroot?>admin/body_types/delete/"+aa;
      } 
}
...
function bulk_active(status)
{
    ...
    if(is_checked)
    {
        ...
        $.post("<?php echo $this->webroot;?>admin/body_types/bulk_active/",data,function(result){
        if(result.trim()==1)    
        {
         location.href="<?php echo $this->webroot; ?>admin/body_types/index";
        }
        else
        {
         location.reload();
...
function bulk_delete()
{
    ...
    
    {
        ...
        if (a)
        {
            ...
            $.post("<?php echo $this->webroot;?>admin/body_types/bulkdelete/",data,function(result){
            if(result.trim()==1)    
            {
             location.href="<?php echo $this->webroot; ?>admin/body_types/index";
            }
