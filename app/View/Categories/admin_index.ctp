  ...
  <div class="wrapper">
   <div class="row">
				<div class="col-sm-9 col-lg-3">
                            ...
                            <a href="<?php echo $this->webroot; ?>admin/categories" class=" widget-hover-effect1" style="text-decoration:none">
                                <button class="btn btn-sm btn-success btn-block" type="button" style="font-size:20px;margin-bottom:10px;">Escort Categories</button>
                            </a>
                            
                 </div>
				<div class="col-sm-9 col-lg-3">
							
                            <a href="<?php echo $this->webroot; ?>admin/services" class=" widget-hover-effect1" style="text-decoration:none">
                                <button class="btn btn-sm btn-info btn-block" type="button" style="font-size:20px;margin-bottom:10px;">Escort Services</button>
                            </a>
                            
                 </div>
                 <div class="col-sm-9 col-lg-4">
							
                            <a href="<?php echo $this->webroot; ?>admin/contact_types" class=" widget-hover-effect1" style="text-decoration:none">
                                <button class="btn btn-sm btn-info btn-block" type="button" style="font-size:20px;margin-bottom:10px;">Escort Contact Instruction</button>
                            </a>
                            
                 </div>
                 <div class="col-sm-9 col-lg-2">
							
                            <a href="<?php echo $this->webroot; ?>admin/languages" class=" widget-hover-effect1" style="text-decoration:none">
...
-->
    ...
    <div class="row gap_row">
      <div class="col-sm-12">
        ...
        <header class="panel-heading">
            All Category Tags  (Total Records: <?php echo $total_category; ?>)
        </header>
           ...
           <?php echo $this->Form->input("advace_search_type",array('type'=>'hidden','value'=>0))?>
            <div class="table-options clearfix" style=" background-color:#fff">
                ...
                <div class="btn-group btn-group-sm pull-right">
					<a id="style-hover" class="btn btn-primary gap_up gap_up_down" title="" data-toggle="tooltip" href="<?php echo $this->webroot;?>admin/categories/add" data-original-title="Add Category">Add New</a>
				</div>
       ...
       <div class="block-title">
        <div class="block-options pull-right form-actions">
            <button class="btn btn-alt btn-sm btn-danger enable-tooltip" id="advanceSearch_close" type="button" data-original-title="Advance Search close"  data-toggle="tooltip" data-placement="top" title="Advance Search Close" onclick="advance_close();"><i class="fa fa-times"></i></button>
             <button class="btn btn-alt btn-sm btn-warning enable-tooltip" onclick="location.href='<?php echo $this->webroot;?>admin/categories'" type="button" data-original-title="Reset Search" data-toggle="tooltip" data-placement="top" title="Reset Search"><i class="fa fa-repeat"></i></button>
        </div>
                               ...
                               <div class="form-inline">
                                    <div class="form-group">
                                        ...
                                        <?php echo $this->Form->input("name",array('type'=>"text",'class'=>'form-control','placeholder'=>'Search by  name...','label'=>false,'div'=>false))?>
                                         ...
                                         <br>
                                         <?php
                                            echo $this->Form->input("active",array('options'=>$ad_types,'class'=>'select-chosen','data-placeholder'=>'Choose a Type..','style'=>'width: 250px','selected'=>!empty($this->request->data['Filter']['is_active'])?$this->request->data['Filter']['is_active']:'','label'=>false,'div'=>false));
	...
	<tr>
        ...
        <tbody>
	<?php foreach ($listcategories as $content):
	    
	    ?>
        <tr class="gradeX">
        <td><input type="checkbox" class="changestatus" value="<?php echo $content["Category"]["id"]; ?>"></td>
        <td><?php echo $content["Category"]["id"]; ?></td>
        <td><?php echo $content["Category"]["name"]; ?></td>
        <td><?php echo $content["Category"]["active"]?"Active":"Inactive"; ?></td>
        <td><?php echo date("Y-m-d",strtotime($content["Category"]["posttime"])); ?></td>
        <td class="text-center">
        <div class="btn-group btn-group-sm">
            <a href="javascript:void(0);" onclick="viewad(<?php echo $content['Category']['id']; ?>)" class="btn btn-default enable-tooltip view-record" data-original-title="View Record"><i class="fa fa-search"></i></a>
            <a href="<?php echo $this->webroot;?>admin/categories/edit/<?php echo $content['Category']['id']; ?>" class="btn btn-default enable-tooltip view-record btn btn-primary" data-original-title="edit Record"><i class="fa fa-pencil" aria-hidden="true"></i></a>                
           <a href="javascript:void(0)" onclick="del(<?php echo $content['Category']['id'];?>)" class="btn btn-default enable-tooltip view-record btn btn-primary" data-original-title="View Record" onclick="bulk_delete()"><i class="fa fa-times" aria-hidden="true"></i></a>


 <?php 
        if($content["Category"]["active"]==0)
        {
        ?>
        <a href="<?php echo $this->webroot; ?>admin/categories/changestatus/<?php echo $content['Category']['id']; ?>/1" class="btn btn-default enable-tooltip view-record" data-original-title="Active" 
        style="background-color:#f7be64;border-color:#f7be64; display: inline-block;"><i class="fa fa-circle-o"></i></a>
        <?php }else{?>
       <a href="<?php echo $this->webroot; ?>admin/categories/changestatus/<?php echo $content['Category']['id']; ?>/0" class="btn btn-default enable-tooltip view-record" data-original-title="Inactive"
...
function viewad(userid){
  //alert(userid);
	$.get('<?php echo($this->webroot)?>admin/categories/view/'+userid,function(data){
...
function del(aa){
  var a=confirm("Are you sure, you want to delete this?")
      if (a)
      {
        location.href="<?php echo $this->webroot?>admin/categories/delete/"+aa;
      } 
}
...
function bulk_active(status)
{
    ...
    if(is_checked)
    {
        ...
        $.post("<?php echo $this->webroot;?>admin/categories/bulk_active/",data,function(result){
        if(result.trim()==1)    
        {
         location.href="<?php echo $this->webroot; ?>admin/categories/index";
        }
        else
        {
         location.reload();
...
function bulk_delete()
{
    ...
    
    {
        ...
        if (a)
        {
            ...
            $.post("<?php echo $this->webroot;?>admin/categories/bulkdelete/",data,function(result){
            if(result.trim()==1)    
            {
             location.href="<?php echo $this->webroot; ?>admin/categories/index";
            }
