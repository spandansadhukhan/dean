    ...
    <div class="row">
				<div class="col-sm-9 col-lg-3">
                            ...
                            <a href="<?php echo $this->webroot; ?>admin/categories" class=" widget-hover-effect1" style="text-decoration:none">
                                <button class="btn btn-sm btn-info btn-block" type="button" style="font-size:20px;margin-bottom:10px;">Escort Categories</button>
                            </a>
				...
				<div class="col-sm-9 col-lg-3">
                            ...
                            <a href="<?php echo $this->webroot; ?>admin/services" class=" widget-hover-effect1" style="text-decoration:none">
                                <button class="btn btn-sm btn-info btn-block" type="button" style="font-size:20px;margin-bottom:10px;">Escort ContactTypes</button>
                            </a>
                 ...
                 <div class="col-sm-9 col-lg-4">
                            ...
                            <a href="<?php echo $this->webroot; ?>admin/contact_types" class=" widget-hover-effect1" style="text-decoration:none">
                                <button class="btn btn-sm btn-success btn-block" type="button" style="font-size:20px;margin-bottom:10px;">Escort Contact Instruction</button>
                            </a>
...
-->
    ...
    <div class="row gap_row">
      <div class="col-sm-12">
        ...
        <header class="panel-heading">
            Contact Instruction  (Total Records: <?php echo $total_instruction; ?>)
        </header>
           ...
           <?php echo $this->Form->input("advace_search_type",array('type'=>'hidden','value'=>0))?>
            <div class="table-options clearfix" style=" background-color:#fff">
                ...
                <div class="btn-group btn-group-sm pull-right">
					<a id="style-hover" class="btn btn-primary gap_up gap_up_down" title="" data-toggle="tooltip" href="<?php echo $this->webroot;?>admin/contact_types/add" data-original-title="Add ContactType">Add New</a>
				</div>
       ...
       <div class="block-title">
        <div class="block-options pull-right form-actions">
            <button class="btn btn-alt btn-sm btn-danger enable-tooltip" id="advanceSearch_close" type="button" data-original-title="Advance Search close"  data-toggle="tooltip" data-placement="top" title="Advance Search Close" onclick="advance_close();"><i class="fa fa-times"></i></button>
             <button class="btn btn-alt btn-sm btn-warning enable-tooltip" onclick="location.href='<?php echo $this->webroot;?>admin/contact_types'" type="button" data-original-title="Reset Search" data-toggle="tooltip" data-placement="top" title="Reset Search"><i class="fa fa-repeat"></i></button>
        </div>
	...
	<tr>
		...
		<th><?php echo $this->Paginator->sort('name','Contact Instruction'); ?></th>
        ...
        <tbody>
	<?php foreach ($contact_instructions as $content):
	    
	    ?>
        <tr class="gradeX">
        <td><input type="checkbox" class="changestatus" value="<?php echo $content["ContactType"]["id"]; ?>"></td>
        <td><?php echo $content["ContactType"]["id"]; ?></td>
        <td><?php echo $content["ContactType"]["name"]; ?></td>
        <td><?php echo $content["ContactType"]["is_active"]?"Active":"Inactive"; ?></td>
        <td><?php echo date("Y-m-d",strtotime($content["ContactType"]["posttime"])); ?></td>
        <td class="text-center">
        <div class="btn-group btn-group-sm">
            <a href="javascript:void(0);" onclick="viewad(<?php echo $content['ContactType']['id']; ?>)" class="btn btn-default enable-tooltip view-record" data-original-title="View Record"><i class="fa fa-search"></i></a>
            <a href="<?php echo $this->webroot;?>admin/contact_types/edit/<?php echo $content['ContactType']['id']; ?>" class="btn btn-default enable-tooltip view-record btn btn-primary" data-original-title="edit Record"><i class="fa fa-pencil" aria-hidden="true"></i></a>                
           <a href="javascript:void(0)" onclick="del(<?php echo $content['ContactType']['id'];?>)" class="btn btn-default enable-tooltip view-record btn btn-primary" data-original-title="View Record" onclick="bulk_delete()"><i class="fa fa-times" aria-hidden="true"></i></a>


 <?php 
        if($content["ContactType"]["is_active"]==0)
        {
        ?>
        <a href="<?php echo $this->webroot; ?>admin/contact_types/changestatus/<?php echo $content['ContactType']['id']; ?>/1" class="btn btn-default enable-tooltip view-record" data-original-title="Active" 
        style="background-color:#f7be64;border-color:#f7be64; display: inline-block;"><i class="fa fa-circle-o"></i></a>
        <?php }else{?>
       <a href="<?php echo $this->webroot; ?>admin/contact_types/changestatus/<?php echo $content['ContactType']['id']; ?>/0" class="btn btn-default enable-tooltip view-record" data-original-title="Inactive"
...
function viewad(userid){
  //alert(userid);
	$.get('<?php echo($this->webroot)?>admin/contact_types/view/'+userid,function(data){
...
function del(aa){
  var a=confirm("Are you sure, you want to delete this?")
      if (a)
      {
        location.href="<?php echo $this->webroot?>admin/contact_types/delete/"+aa;
      } 
}
...
function bulk_active(status)
{
    ...
    if(is_checked)
    {
        ...
        $.post("<?php echo $this->webroot;?>admin/contact_types/bulk_active/",data,function(result){
        if(result.trim()==1)    
        {
         location.href="<?php echo $this->webroot; ?>admin/contact_types/index";
        }
        else
        {
         location.reload();
...
function bulk_delete()
{
    ...
    
    {
        ...
        if (a)
        {
            ...
            $.post("<?php echo $this->webroot;?>admin/contact_types/bulkdelete/",data,function(result){
            if(result.trim()==1)    
            {
             location.href="<?php echo $this->webroot; ?>admin/contact_types/index";
            }
    ...
    <div class="modal-dialog modal-md">
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title">ContactType Detail </h4>
        </div>
