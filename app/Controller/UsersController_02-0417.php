    ...
    public function index($id = null) {
        ...
        if($this->request->data)
     {
        ...
        $name = $this->request->data['user_name'];
              $this->paginate = array(
        'conditions' => array('User.user_type' => 'E','User.id !=' => $this->Session->read('fuid'),'User.name LIKE'=>$name,'User.country_id'=>230,'User.city_id'=>$this->request->data['city']),
        'limit' => 8,
        'order' => array(
        'User.is_sort' => 'asc'
        )
...
//pr($this->request->data);exit;
        ...
        elseif(isset($this->request->data['city']) && $this->request->data['city'] !='')
        {

                
             $options = array('conditions' => array('User. city_id' => $this->request->data['city'],'User.user_type' => 'E'));
             $escorts = $this->User->find('all', $options);
        }
       ...
       // pr($showonline); exit;
        ...
        $this->set(compact('services','genders','agearr','category', 'escorts','locationsarray', 'surubsarrays','showonline', 'showfeature','shownewescorts','showescortofweek','showcountry'));
    }
...
public function membershippayment() {
    ...
    {
        $this->layout=false;
        $is_paid= isset($_REQUEST['is_paid'])?$_REQUEST['is_paid']:'';
        $is_active= isset($_REQUEST['is_active'])?$_REQUEST['is_active']:'';
        $gender= isset($_REQUEST['gender'])?$_REQUEST['gender']:'';
        $is_loggedin= isset($_REQUEST['is_loggedin'])?$_REQUEST['is_loggedin']:'';
