...
class AppController extends Controller {
	...
	public function beforeFilter() {
        ...
        public function beforeRender() {
                ...
                if($userdetails['User']['user_type']=='E')
                {
                   $this->loadModel('Membership');
                   $membership=$this->Membership->find("first",array('conditions'=>array("Membership.id"=>$userdetails["User"]["membership_id"])));
                   $permissions= json_decode($membership['Membership']['allowed_features']);
                   
                }
                
                if($userdetails['User']['user_type']=='A' or $userdetails['User']['user_type']=='C' or $userdetails['User']['user_type']=='P')
                {
                   $this->loadModel('AgentMembership');
                   $membership=$this->AgentMembership->find("first",array('conditions'=>array("AgentMembership.id"=>$userdetails["User"]["membership_id"])));
                   $permissions= json_decode($membership['AgentMembership']['allowed_features']);
                   
                }
                //pr($permissions);exit;
                $this->set(compact('userdetails','permissions','count_video'));
                }
                ...
                $optionsfeature = array('conditions' => array('User.user_type' => 'E','User.is_featured ' => 1),'limit'=>8,'order'=>"rand()");
                $showfeature = $this->User->find('all', $optionsfeature);
                
                $this->loadModel("Advertisement");
                $options=array("conditions"=>array("Advertisement.end_date >="=>date("Y-m-d")),"order"=>array("rand()"),"limit"=>4);
                $all_adds=$this->Advertisement->find("all",$options);
                $total_escort=$this->User->find('count',array('conditions'=>array("User.user_type"=>'E')));
                //$this->set(compact('categories'));

                $this->set(compact('total_escort','escortslider','countries','states','haircolors','eyecolors','bodytypes','escorttypes','user_status','directions', 'banners','showfeature','all_adds'));
        }
