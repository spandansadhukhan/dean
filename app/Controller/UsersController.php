...
class UsersController extends AppController {
    ...
    public function login_new() 
    {
        ...
        if (!$loginuser) {
         ...
         //$this->redirect(array('action'=>'index'))
            ...
            $this->Session->setFlash(__('You are not a active user.', 'default', array('class' => 'error'))); 
            $this->logout();
            } else {
                ...
                if ($loginuser['User']['user_type'] == "E") {
                    ...
                    $this->User->id = $loginuser['User']['id'];
                    $this->User->saveField('is_loggedin', 1, false);
                    //$this->redirect(array('controller'=>'escorts','action'=>'escortdashboard'));
                   ...
                   $this->Session->setFlash(__('You have been successfully logged in.', 'default', array('class' => 'success')));
                    ...
                    $this->User->saveField('last_login', $time_now, false);
                    $this->User->saveField('is_loggedin', 1, false);
                    //$this->redirect(array('controller'=>'agencies','action'=>'agencydashboard'));
                    $this->Session->setFlash(__('You have been successfully logged in.', 'default', array('class' => 'success')));
                    $this->redirect( Router::url( $this->referer(), true ) );         
                    ...
                    $this->User->saveField('last_login', $time_now, false);
                    $this->User->saveField('is_loggedin', 1, false);
                    //$this->redirect(array('controller'=>'clubs','action'=>'clubdashboard'));
                    $this->Session->setFlash(__('You have been successfully logged in.', 'default', array('class' => 'success')));
                    $this->redirect( Router::url( $this->referer(), true ) );         
                    ...
                    $this->User->saveField('last_login', $time_now, false);
                    $this->User->saveField('is_loggedin', 1, false);
                    //$this->redirect(array('controller'=>'parlors','action'=>'parlordashboard'));
                   $this->Session->setFlash(__('You have been successfully logged in.', 'default', array('class' => 'success')));
                    $this->redirect( Router::url( $this->referer(), true ) );         
                    ...
                    $this->User->saveField('last_login', $time_now, false);
                    $this->User->saveField('is_loggedin', 1, false);
                    //$this->redirect(array('controller'=>'strippers','action'=>'stripperdashboard'));
                    $this->Session->setFlash(__('You have been successfully logged in.', 'default', array('class' => 'success')));
                    $this->redirect( Router::url( $this->referer(), true ) );         
    ...
    public function admin_subadmin() {
        ...
        $this->Paginator->settings = array(
            ...
            'order' => $order_by,
            'limit' => $limit,
            'fields' => array('User.id', 'User.name', 'User.username', 'User.email', 'User.department', 'User.join_date', 'User.profile_image','User.is_active')
        );

        $total_user = $this->User->find('count', array('conditions' => $conditions));
        $total_active_user = $this->User->find('count', array('conditions' => array('User.is_admin'=>1 ,'User.is_active'=>1,'User.id !=' =>1)));
        $total_inactive_user = $this->User->find('count', array('conditions' =>  array('User.is_admin'=>1 ,'User.is_active'=>0,'User.id !=' =>1)));
    ...
    public function admin_subadminactive($id = null) {
    ...
    public function admin_subadmindelete($id = null) {
    ...
    public function admin_agentactive($id = null) {
...
public function admin_addwebsitetemplate() {
        
        $userid = $this->Session->read('userid');
		if(!isset($userid) && $userid==''){ $this->redirect('/admin'); }
       
        if ($this->request->is('post')) {
            $this->WebsiteTemplate->create();
            
             
                if (!empty($this->request->data['WebsiteTemplate']['template']['name'])) {
                    $pathpart = pathinfo($this->request->data['WebsiteTemplate']['template']['name']);
                    $ext = $pathpart['extension'];
                    $extensionValid = array('jpg', 'jpeg', 'png', 'gif');
                    if (in_array(strtolower($ext), $extensionValid)) {
                        $uploadFolder = "img";
                        ...
                        $filename = uniqid() . '.' . $ext;
                        $full_flg_path = $uploadPath . '/' . $filename;
                        move_uploaded_file($this->request->data['WebsiteTemplate']['template']['tmp_name'], $full_flg_path);
                        $this->request->data['WebsiteTemplate']['template'] = $filename;
                    } else {
                        $this->Session->setFlash(__('Invalid image type.'));
                        return $this->redirect(array('action' => 'websitetemplates'));
                    }
                } else {
                    $this->request->data['WebsiteTemplate']['template'] = '';
                }

                

                
                if ($this->WebsiteTemplate->save($this->request->data)) {

                    $this->Session->setFlash('The WebsiteTemplate has been saved.', 'default', array('class' => 'success'));

                    return $this->redirect(['action' => 'websitetemplates']);
                } else {
                    $this->Session->setFlash(__('The WebsiteTemplate could not be saved. Please, try again.'));
                }
    ...
    public function admin_editwebsitetemplate($id = null) {
        ...
        if ($this->request->is(array('post', 'put'))) {
            ...
            $options = array('conditions' => array('WebsiteTemplate.' . $this->WebsiteTemplate->primaryKey => $id));
            $this->request->data = $this->WebsiteTemplate->find('first', $options);
        }
...
//            }
//            
//            $secret = '6LcBtBsUAAAAANgVXbl9dtUIfEVnInvBUZy4vdGo';
//
...
//            {
//                $this->Session->setFlash(__('Please Click captcha.'));
//                $flag = false;
//            }
//            
//            
...
//                $this->request->data['User']['first_name'] = !empty($raw_name_arr[0])?$raw_name_arr[0]:'';
//                $this->request->data['User']['last_name'] = !empty($raw_name_arr[1])?$raw_name_arr[1]:'';
...
//                if ($this->User->save($this->request->data)) {
//
//                     $txtpass=$this->request->data['password'];
//
//
...
//
//                     $msg_body =str_replace(array('[NAME]','[EMAIL]','[PASSWORD]','[PHONE]'),array($lastInsetred['User']['name'],$lastInsetred['User']['email'],$txtpass,$lastInsetred['User']['phone_no']),$EmailTemplate['EmailTemplate']['content']);
//
//                     $from = 'Admin';
...
//        $countries2 = $this->Country->find('list', array('conditions' => array('Country.is_active' => 1,'Country.id'=>158), 'order' => array('Country.name' => 'ASC')));
//        //pr($countries);exit;
//        $this->loadModel('State');
...
//        $this->set(compact('title_for_layout', 'countries2', 'state', 'utype', 'city','locationsarray'));
//    }
        ...
        if ($this->request->is('post')) {
           ...
           //pr($this->request->data); exit;
            ...
            if ($flag) {
                ...
                $this->request->data['User']['is_admin'] = 0;
                $this->request->data['User']['join_date'] = date('Y-m-d');
                $this->request->data['User']['notes'] = $this->request->data['notes'];
                $this->request->data['User']['is_photographer'] = $this->request->data['is_photographer'];
                $this->request->data['User']['is_freeemail'] = $this->request->data['is_freeemail'];
                $this->request->data['User']['business_name'] = $this->request->data['business_name'];
    ...
    public function logout() {
        ...
        $this->User->saveField('is_loggedin', 0, false);
    ...
    public function admin_templateview($id = null) {
         $userid = $this->Session->read('userid');
          if(!isset($userid) && $userid==''){
          $this->redirect('/admin');
          }
        ...
        $options = array('conditions' => array('WebsiteTemplate.' . $this->WebsiteTemplate->primaryKey => $id));
        $user = $this->WebsiteTemplate->find('first', $options);
        //pr($user);exit;
        if ($user['WebsiteTemplate']['is_active'] == '0') {
            $status = 'Inactive';
        } else {
            $status = 'Active';
        }
        if ($user['WebsiteTemplate']['is_paid'] == 0) {
            $paid = 'Free';
        } else {
            $paid = 'Paid';
        }
        $name=$user['WebsiteTemplate']['name'];
       
         $image=Configure::read('SITE_URL')."img/".$user["WebsiteTemplate"]["template"];
        

        //  $this->loadModel('Multimageupload');
        //  $multimg = $this->Multimageupload->find('all',array('conditions'=>array('Multimageupload.user_id'=>$userid)));
        $data = '<tr><td style="width:55%;">Id</td><td>' . $user['WebsiteTemplate']['id'] . '</td></tr>'
                . '<tr><td>Name</td><td>' . $name . '</td></tr>'
                . '<tr><td>Template</td><td><img src='.$image.' height="200px" width="200px"></td></tr>'
                . '<tr><td>Type</td><td>' . $paid . '</td></tr>'
                . '<tr><td>Price</td><td>' . $user['WebsiteTemplate']['price'] . '</td></tr>'
                . '<tr><td>Status</td><td>' . $status . '</td></tr>';
        //pr($user);exit;
        echo $data;
        exit;
...
//pr($images);exit;
    ...
    public function index($id = null) {
        ...
        $options11 = array('conditions' => array('Category.active' => 1),'fields'=>array('Category.id','Category.name'));
        $categories = $this->Category->find('all', $options11);
...
//        //country
          $showcountry = $this->Country->find('list', array('fields'=>array('Country.id','Country.name'),'conditions'=>array('Country.id'=>158),'order' => 'Country.id ASC')); 
           ...
           {
              $numb = $this->User->find('count',array('conditions'=>array('FIND_IN_SET(\''. $cat['Category']['id'] .'\',User.category_id)','User.user_type'=>'E')));
              $category_wise_escorts[] = array('count'=>$numb,'id'=>$cat['Category']['id'],'name'=>$cat['Category']['name']);
           }
            $genders=array('M'=>'Male','F'=>'Female','T'=>'Trans');
     ...
     $heights=array("any"=>"Any","140-149"=>"140cm-149cm","150-159"=>"150cm-159cm","160-169"=>"160cm-169cm","170-179"=>"170cm-179cm"
        ...
        $this->set(compact('verifieds',"showing_face","category_wise_escorts","heights",'haircolors','nationalities','availabilities','cups','busts','services','genders','agearr','category', 'escorts','locationsarray', 'surubsarrays','showonline', 'showfeature','shownewescorts','showescortofweek','showcountry'));
    }
    ...
    public function admin_websitetemplates() {
		...
		if(!isset($userid) && $userid==''){ $this->redirect('/admin'); }
            ...
            $total_user = $this->WebsiteTemplate->find('count');
            $total_active_user = $this->WebsiteTemplate->find('count', array('conditions' => array('WebsiteTemplate.is_active'=>1)));
        $total_inactive_user = $this->WebsiteTemplate->find('count', array('conditions' =>  array('WebsiteTemplate.is_active'=>0)));
            
            $this->set(compact('name','total_user','total_active_user','total_inactive_user'));
    ...
    public function admin_activetemplate($id = null) {

        
        $checkuser = $this->WebsiteTemplate->find('first', array('conditions' => array('WebsiteTemplate.id' => $id)));
        if ($checkuser['WebsiteTemplate']['is_active'] == 1) {
            $status = 0;
        } elseif ($checkuser['WebsiteTemplate']['is_active'] == '') {
            $status = 1;
        }

        $this->WebsiteTemplate->updateAll(array('WebsiteTemplate.is_active' => "'$status'"), array('WebsiteTemplate.id' => $id));
    ...
    public function escorttours($id=null) {
        // $userid = $this->Session->read('fuid');
        // if (!isset($userid)) {
        //     $this->redirect('/');
        // }
        ...
        $conditions["User.user_type"]='E';
        $conditions["User.id !="]=$this->Session->read('fuid');
        $conditions["Escorttour.tour_to >="]=date('Y-m-d');
        ...
        foreach($category as $key=>$cat)
        {
          ...
          $condition_new[0]='FIND_IN_SET(\''. $cat['Category']['id'] .'\',User.category_id)';  
          $condition_new['User.user_type']='E';  
          $condition_new['Escorttour.tour_to >=']=date('Y-m-d');  
          $options=array('conditions'=>$condition_new);
          $numb = $this->Escorttour->find('count',$options);
        ...
        {
            $id= base64_decode($id);
            $conditions[0]='FIND_IN_SET(\''. $id .'\',User.category_id)';
        ...
        $escorttours=$this->Paginator->paginate('Escorttour'); 
        ...
        $this->loadModel('Availability');
         $availability2 = $this->Availability->find('list');
           ...
           $verifieds=array("any"=>"Any",'Yes'=>'Yes','No'=>'No');
           $this->set(compact('verifieds','list',"showing_face","category_wise_escorts","heights",'haircolors','nationalities','availabilities','cups','busts','services','genders','agearr','category', 'escorts','locationsarray', 'surubsarrays','showonline', 'showfeature','shownewescorts','showescortofweek','showcountry'));
    ...
    public function escortlist($type = null,$id=null) {
                        ...
                        if(!in_array($param_name, array('page','sort','direction','limit')))
                            {
                                    ...
                                    else if($param_name=='gender' and !empty($value))
                                    {
     ...
     $heights=array("any"=>"Any","140-149"=>"140cm-149cm","150-159"=>"150cm-159cm","160-169"=>"160cm-169cm","170-179"=>"170cm-179cm"
        ...
        $this->set(compact('verifieds','list',"showing_face","category_wise_escorts","heights",'haircolors','nationalities','availabilities','cups','busts','services','genders','agearr','category', 'escorts','locationsarray', 'surubsarrays','showonline', 'showfeature','shownewescorts','showescortofweek','showcountry','state','city'));
...
public function membershippayment() {
    ...
    public function admin_membership_feature($id = null){
        ...
        $data= json_encode($this->request->data["Membership"]);
     ...
     $options1 = array('conditions' => array('Membership.id' => $id));
     $features=$this->Membership->find("first",$options1);
     $allowed_featurs= json_decode($features["Membership"]["allowed_features"]);  
            $this->set(compact("allowed_featurs","features"));
    ...
    function ajax_search()
    {
        ...
        $conditions1="";
        foreach ($this->request->data['Filter'] as $param_name =>$value){
        if($param_name=='age' and !empty($value) and $value!='any')
        {
        ...
        if(!empty($this->request->data['category_id1']))
        {
            $conditions["OR"][]='FIND_IN_SET(\''.$this->request->data['category_id1'].'\',User.category_id)';
        }
        if(!empty($this->request->data['category_id2']))
        {
            $conditions["OR"][]=' FIND_IN_SET(\''.$this->request->data['category_id2'].'\',User.category_id)';
        }
        if(!empty($this->request->data['category_id3']))
        {
            $conditions["OR"][]=' FIND_IN_SET(\''.$this->request->data['category_id3'].'\',User.category_id)';
        }
        if(!empty($this->request->data['category_id4']))
        {
            $conditions["OR"][]='FIND_IN_SET(\''.$this->request->data['category_id4'].'\',User.category_id)';
        }
        if(!empty($this->request->data['category_id5']))
        {
            $conditions["OR"][]='FIND_IN_SET(\''.$this->request->data['category_id5'].'\',User.category_id)';
        }
        if(!empty($this->request->data['category_id6']))
        {
            $conditions["OR"][]='FIND_IN_SET(\''.$this->request->data['category_id6'].'\',User.category_id)';
        }
        if(!empty($this->request->data['category_id7']))
        {
            $conditions["OR"][]='FIND_IN_SET(\''.$this->request->data['category_id7'].'\',User.category_id)';
        }
        if(!empty($this->request->data['category_id8']))
        {
            $conditions["OR"][]='FIND_IN_SET(\''.$this->request->data['category_id8'].'\',User.category_id)';
        }
        if(!empty($this->request->data['category_id9']))
        {
            $conditions["OR"][]='FIND_IN_SET(\''.$this->request->data['category_id9'].'\',User.category_id)';
        }
        if(!empty($this->request->data['category_id10']))
        {
            $conditions["OR"][]='FIND_IN_SET(\''.$this->request->data['category_id10'].'\',User.category_id)';
        }
        if(!empty($this->request->data['category_id11']))
        {
            $conditions["OR"][]='FIND_IN_SET(\''.$this->request->data['category_id11'].'\',User.category_id)';
        }
        if(!empty($this->request->data['search_name']))
        {
            $conditions["User.name LIKE"]='%'.$this->request->data['search_name'].'%';
        }
        $escorts=$this->User->find("all",array("conditions"=>$conditions));
        $this->set(compact('escorts'));    
    }
    
    function ajax_searchonline()
    {
        $this->layout=false;
        $this->loadModel("UserVideo");
        $conditions['User.is_admin']=0;
     ...
     function ajax_searchtour()
    {
        ...
        $escorts=$this->Escorttour->find("all",array("conditions"=>$conditions));
        $this->set(compact('escorts'));    
    }
    ...
    {
        $this->loadModel("Location");
        $options=array("fields"=>array("Location.name"),"conditions"=>array("Location.id"=>$id));
        $loc=$this->Location->find("first",$options);
        return !empty($loc["Location"]["name"])?$loc["Location"]["name"]:"";
    ...
    {
       $this->loadModel('Membership');
       $meberships=$this->Membership->find("all",array("conditions"=>array("Membership.is_active"=>1)));
       foreach ($meberships as $mebership)
       {
           $roles=json_decode($mebership["Membership"]["allowed_features"]);
           
           if($roles->$key==1)
           {
               $outs[]=$mebership["Membership"]["name"];
           }    
       }
       $output= implode(",", $outs);
    ...
    function  convert_json()
    {
        $array=array(
        "uploadphotos"=>4,
        "add_escort_profile"=>6,
        "upload_video"=>0,
        "my_profile"=>500,
        "customer_blacklist"=>1,
        "direct_booking"=>0,
        "contact_info"=>2,
        "prioritylisting"=>0,
        "receive_review"=>0,
...
public function admin_activeescort($id = null) {
    ...
    public function admin_verifyescortemail($id = null) {

        
        $checkuser = $this->User->find('first', array('conditions' => array('User.id' => $id)));
        if ($checkuser['User']['is_email_verified'] == 1) {
            $status = 0;
        } elseif ($checkuser['User']['is_email_verified'] == '') {
            $status = 1;
        }

        $this->User->updateAll(array('User.is_email_verified' => "'$status'"), array('User.id' => $id));
        $this->redirect(array('controller'=>'escorts','action' => 'index'));
    }
